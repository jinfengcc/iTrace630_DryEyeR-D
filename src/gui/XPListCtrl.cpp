...
CXPListCtrl::CXPListCtrl()
{
	...
	m_FelRowColor = 0x00ffff00;//0x00efdfcf; 531
	m_HotRowColor = 0x00efffdf;//0x00ffefcf;
}

//***************************************************************************************

void CXPListCtrl::CreateWnd(ulong Style, const RECT& Rect, CWnd* pParentWnd, uint ID)
{
...
void CXPListCtrl::InsertColumn(const int n, const wchar_t *Name, const int Width)
{
	LVCOLUMN Column;
...
void CXPListCtrl::SetSortItem(const int sort_col)
{
...
void CXPListCtrl::OnLButtonDown(uint nFlags, CPoint Point)
{
...
void CXPListCtrl::OnRButtonDown(uint nFlags, CPoint Point)
{
	int row = HitTest(Point);
	if (row == -1) return;
	...
	this->GetParent()->SendMessage(WM_NOTIFY, NM_RCLICK, reinterpret_cast<LPARAM>(&nmh));
}

//***************************************************************************************

void CXPListCtrl::OnLButtonDblClk(uint nFlags, CPoint Point)
{
	int row = HitTest(Point);
	if (row == -1) return;
...
void CXPListCtrl::OnRButtonDblClk(uint nFlags, CPoint Point)
{
	int row = HitTest(Point);
	if (row == -1) return;
...
void CXPListCtrl::MeasureItem(MEASUREITEMSTRUCT* pMIS)
{
...
void CXPListCtrl::DrawItem(DRAWITEMSTRUCT* pDIS)
{
	CMDC DC;
	DC.Attach(pDIS->hDC);
	...
	for (int col = 0; col < m_HeaderCtrl.m_num_cols; col++)
	{
		...
		// GetSubItemRect ���������� ��� col==0 rectangle ���� ������, � �� ������� ������
		GetSubItemRect(pDIS->itemID, col, LVIR_BOUNDS, Rect);
		Rect.right = Rect.left + Column.cx - 1;
		Rect.left += 3;
		if (col == 0 && m_ColorNum[pDIS->itemID] == 1)
		{
			if (Selected) DC.WriteText(GetItemText(pDIS->itemID, col), Rect, *m_pFont, YELLOW, 0);
			else DC.WriteText(GetItemText(pDIS->itemID, col), Rect, *m_pFont, MAGENTA, 0);
		}
		else DC.WriteText(GetItemText(pDIS->itemID, col), Rect, *m_pFont, TextColor, 0);
	}
