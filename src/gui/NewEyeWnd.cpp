...
CNewEyeWnd::CNewEyeWnd(RECT& Rect, CWnd* pWnd, int Scale) : CDispWnd(Rect, pWnd)
{
	...
	m_Scale = Scale;//521
	m_ScaleRatio = real_t(5000) / (real_t)m_Scale;
...
void CNewEyeWnd::Grid()
{
...

	{
		...
		for (real_t um = -m_Scale; um < m_Scale + 1.0; um += 1000.0) {
			real_t sq = sqrt(sqr(m_Scale) - sqr(um));
			// horizontal lines
		...
		for (int a = 0; a < 360; a += 5) {
			real_t r1_um = m_Scale + 100.0 / m_ScaleRatio;
			int x1 = intRound(m_cx + m_x_px_um * r1_um * COS[a]);
			int y1 = intRound(m_cy - m_y_px_um * r1_um * SIN[a]);
			real_t r2_um = (a % 15) == 0 ? m_Scale + 400.0 / m_ScaleRatio : m_Scale + 250.0 / m_ScaleRatio;
			...
			if (a % 30 == 0) {
				real_t r_um = m_Scale + 900.0 / m_ScaleRatio;
