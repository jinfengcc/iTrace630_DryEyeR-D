...
BEGIN_MESSAGE_MAP(CWFVrfTestWnd, CSumWnd)
...
CWFVrfTestWnd::CWFVrfTestWnd(CWnd* pWnd, RECT& WndRect, CPatient* pPatient, CWFExam* pWFExam, CWndSettings* pWndSettings) :
	CSumWnd(pWnd, WndRect, pPatient, pWndSettings)
{
	CBusyCursor Cursor;

	m_WFVrfTestSumWnd = TRUE;
...
void CWFVrfTestWnd::OnSizeLargeItemClicked()
{
...
void CWFVrfTestWnd::RepaintMemDC()
{
	CSumWnd::RepaintMemDC();

	WFExamLargeInfo(m_pWFExam, m_Rect[4], 1);
...
void CWFVrfTestWnd::CreateChildWnd()
{
...
void CWFVrfTestWnd::OnShot1ButtonClicked()
{
...
void CWFVrfTestWnd::OnShot2ButtonClicked()
{
	int NumShots = m_pWFExam->GetNumShots();
...
void CWFVrfTestWnd::ChangeShot(int shot)
{
...
void CWFVrfTestWnd::OnPrevButtonClicked()
{
	m_pWFExam->PrvPoint();
	ChangePoint(FALSE);
}
...
void CWFVrfTestWnd::OnNextButtonClicked()
{
	m_pWFExam->NxtPoint();
	ChangePoint(FALSE);
}
...
void CWFVrfTestWnd::ChangePoint(BOOL ChangeShot)
{
...
void CWFVrfTestWnd::OnAccept(BOOL accept)
{
	CScanPoint* pPoint = m_pWFExam->m_Points.Get(m_pWFExam->m_CurPoint);
	pPoint->m_Bad = !accept;
...
void CWFVrfTestWnd::OnEditLimbusItemClicked()
{
...
void CWFVrfTestWnd::OnEditLensDotsItemClicked()
{
	CEyeImage* pImage = &m_pWFExam->m_Image;
...
void CWFVrfTestWnd::OnSelEyeImgItemClicked()
{
...
void CWFVrfTestWnd::OnShowDefaultItemClicked()
{
	CBusyCursor Cursor;
	...
	if (m_d == 0)//WF Point display
	{
		...
		m_pWndSettings[0].m_MapShowLimbus = FALSE;
