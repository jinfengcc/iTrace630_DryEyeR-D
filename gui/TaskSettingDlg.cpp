...
BEGIN_MESSAGE_MAP(CTaskSettinigDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_DYSFUN_CHECK, OnDysfunClicked)//531
	ON_BN_CLICKED(IDC_ASTISOU_CHECK, OnAstiSoufunClicked)//531	
...
CTaskSettinigDlg::CTaskSettinigDlg(CWnd* pParentWnd, CTaskSetting OriTask, int WhichTask) :
	CDialog(IDD_TASK_SETTING_DLG, pParentWnd)
{
	m_OriTask = OriTask;
...
void CTaskSettinigDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_DYSFUN_CHECK, m_Dysfun_Check); //531 Dysfunctional Analysis
	DDX_Control(pDX, IDC_ASTISOU_CHECK, m_AstiSou_Check); //531 Astigmastim Source
	...
	DDX_Control(pDX, IDC_DYSFUN_TEXT, m_DysFun_Text); //531 Dysfunctional Analysis
	DDX_Control(pDX, IDC_ASTISOU_TEXT, m_Astisou_Text); //531 Astigmastim Source
														//Done
	...
	DDX_Control(pDX, IDC_ORDER_TEXT26, m_Order_Text[25]); //n
	DDX_Control(pDX, IDC_ORDER_TEXT27, m_Order_Text[26]); //531
	DDX_Control(pDX, IDC_ORDER_TEXT28, m_Order_Text[27]); //531
}
//***************************************************************************************

BOOL CTaskSettinigDlg::OnInitDialog()
{
	...
	else
	{
		...
		// OnDisableClicked();//531
	}
	m_TaskEn_Radio.ShowWindow(FALSE);//531
	m_TaskDis_Radio.ShowWindow(FALSE);//531

	if (m_isHoya)
	{
		m_Dysfun_Check.ShowWindow(SW_HIDE);
		m_DysFun_Text.ShowWindow(SW_HIDE);
	}
	...
	if (Enable)
	{
		...
		else
		{
			...
			else
			{
				...
				strncpy(sa, (const char*)m_SettedTask.m_ButtonName, 25);
				TaskName = sa;

				if (s != TaskName)
				{
					G_As = s;
					strncpy(&m_SettedTask.m_ButtonName[0], G_As, 25);
				}
			}
		}
	}
...
void CTaskSettinigDlg::OnDefault()
{
	m_t = 0;
...
HBRUSH CTaskSettinigDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CTaskSettinigDlg::OnCheckBoxClick(int whichone)
{
	m_SetDefault = FALSE;
	m_Changed = TRUE;
...
void CTaskSettinigDlg::SetDlg(CTaskSetting Task)
{
	m_TaskName_Edit.SetWindowText((LPCTSTR)Task.m_ButtonName);
	...
	m_Dysfun_Check.SetCheck(FALSE);//531
	m_AstiSou_Check.SetCheck(FALSE);//531
...
int CTaskSettinigDlg::GetNStr(int n)
{
	CString s;
...
//***************************************************************************************
CString CTaskSettinigDlg::GetStrN(int n)
{
	CString s;
...
void CTaskSettinigDlg::OnEnableClicked()
{
	if (m_Enable) return;
	else
	{
...
void CTaskSettinigDlg::OnDisableClicked()
{
	if (!m_Enable) return;
	else
	{
...
void CTaskSettinigDlg::SetDlgAble()
{
	...
	m_Dysfun_Check.EnableWindow(m_Enable);//531
	m_AstiSou_Check.EnableWindow(m_Enable);//531
...
void CTaskSettinigDlg::OnLButtonDown(uint nFlags, CPoint Point)
{
	if (!m_Enable) return;
...
void CTaskSettinigDlg::CreateOriOrder()
{
	for (int i = 0; i < 28; i++)
	{
		m_Order_Int[i] = 0;
...
void CTaskSettinigDlg::GetDlg()
{
	...
	//531 Done
	//531
	if (m_AstiSou_Check.GetCheck()) //Dysfunctional Lens Analysis
	{
		t = GetNStr(27) - 1;
...
BOOL CTaskSettinigDlg::CheckDefEdit()
{
	CString s;
	for (int i = 0; i < 8; i++)
	{
...
//********************************************************************************
CTaskSetting CTaskSettinigDlg::SetDefTask1()
{
	...
	CStringA ADef_Name(Def_Name);
	strncpy(&DefTask.m_ButtonName[0], ADef_Name, 25);
...
//********************************************************************************************
CTaskSetting CTaskSettinigDlg::SetDefTask2()
{
	CTaskSetting DefTask;
...
//********************************************************************************
CTaskSetting CTaskSettinigDlg::SetDefTask0()
{
	CTaskSetting DefTask;
	...
	DefTask.m_SubButton[4] = TYPE_NONA;//531
	DefTask.m_SubButton[5] = TYPE_NONA;//531
	DefTask.m_SubButton[6] = TYPE_NONA;
	DefTask.m_SubButton[7] = TYPE_NONA;
	DefTask.m_SubButton[8] = TYPE_NONA;
	DefTask.m_SubButton[9] = TYPE_NONA;
...
//********************************************************************************
CTaskSetting CTaskSettinigDlg::SetDefTask3()
{
	CTaskSetting DefTask;
...
//********************************************************************************
CTaskSetting CTaskSettinigDlg::SetDefTask4()
{
	CTaskSetting DefTask;
