...
BEGIN_MESSAGE_MAP(CFarLtrWnd, CDispWnd)
...
CFarLtrWnd::CFarLtrWnd(RECT& Rect, CWnd* pWnd) : CDispWnd(Rect, pWnd)
{
	m_Zoom = 400;
}
...
void CFarLtrWnd::CreateZoomButtons()
{
	RECT Rect;
	CMRgn Rgn;
...
void CFarLtrWnd::OnZoomInButtonClicked()
{
	switch (m_Zoom) {
...
void CFarLtrWnd::OnZoomOutButtonClicked()
{
	switch (m_Zoom)
	{
...
void CFarLtrWnd::Letter()
{
	...
	if (m_DysVal >= 0 && m_DysVal <= 10)
	{
		...
		//601
		if (m_DysVal == 10.00) ::SetRect(&Rect, x1 - ValFontSize - ValFontSize, y0 + 8, x0, y0 + 8 + ValFontSize);//601
																												  //601 
		else
		{
			if (m_DysVal <= 0.8)
