...
BEGIN_MESSAGE_MAP(CEyeWnd, CDispWnd)
...
CEyeWnd::CEyeWnd(RECT& Rect, CWnd* pWnd) : CDispWnd(Rect, pWnd)
{
	m_pWFExam = NULL;
	m_pCTExam = NULL;
...
void CEyeWnd::CreateCentButtons()
{
	RECT Rect;
	CMRgn Rgn;
...
void CEyeWnd::CreateZoomButtons()
{
	RECT Rect;
	CMRgn Rgn;
...
void CEyeWnd::CreateOKulixButtons()
{
	...
	int w = intRound(0.058 * m_w); if (w < 20) w = 20;
	...
	int b = m_h - 5;
	::SetRect(&Rect, l, b - w, l + w, b);
	m_OkulixButton.Create(_T(""), _T(""), WS_CHILD, Rect, this, IDC_Okulix_Btn);
	m_OkulixButton.SetImage(m_SizeSmall ?
		(W == 1920 ? IDR_OKULIX_3 : W == 1600 ? IDR_OKULIX_2 : IDR_OKULIX_1) :
		(W == 1920 ? IDR_OKULIX_6 : W == 1600 ? IDR_OKULIX_5 : IDR_OKULIX_4));
	m_OkulixButton.SetBk(FALSE);
	m_OkulixButton.SetBkColor(BLACK);
	m_OkulixButton.SetTip("Okulix");
	Rgn.SetRectRgn(&Rect);
	m_Rgn.CombineRgn(&m_Rgn, &Rgn, RGN_DIFF);

	m_OkulixButton.ShowWindow(SW_SHOW);
}

//***************************************************************************************

void CEyeWnd::RepaintMemDC()
{
	ClearMemDC();
...
void CEyeWnd::Legend()
{
	...
	for (int i = 0; i < m_NumColors; i++) {
		...
		if (m_Type == TYPE_TOPM)
		{
			int vVal = intRound(v * 100.0);
			s.Format(_T("%i"), vVal);
			s += "% ";
			//s.Format(f, v);
		}//
		else if (m_Type == TYPE_CAXM || m_Type == TYPE_CTNM || m_Type == TYPE_CRFM) {
			s.Format(f, v);
		}
		else {
			s.Format(f, fabs(v));
...
void CEyeWnd::Grid()
{
	...
	// circle
	int rx = intRound(5000.0 * m_x_px_um);
	...
	for (int a = 0; a < 360; a += 5) {
		...
		if (a % 30 == 0) {
			...
			//531
			if (!(m_Type == TYPE_TOPM && a == 90 && m_LTLabel[0] == "TOTAL EYE - Opacity Map"))
			{
				if (a == 90) //
				{
					if (m_DonotShow90Degree == TRUE)
					{

					}
					else m_MemDC.WriteRotatedText(s, x, y, a - 90, FontSize, 400, "Arial", white, NOCOLOR);
				}//
				else	m_MemDC.WriteRotatedText(s, x, y, a - 90, FontSize, 400, "Arial", white, NOCOLOR);
			}
			//531 Done
...
//***************************************************************************************
//
void CEyeWnd::BlueAstigmatismAxes()
{
	if (m_Type != TYPE_TWFM &&
		m_Type != TYPE_CWFM &&
		m_Type != TYPE_IWFM &&
	...
	if (m_Method3D) return;
	if (!m_MapShowMap) return;
	if (m_MapShowBLUEAstigmatismAxes != TRUE) return;
	if (m_MapShowBLUEAstigmatismAxes != TRUE && m_MapShowBLUEAstigmatismAxes != FALSE) return;
	if (m_astigm_axis == INVALID_VALUE) return;
	...
	for (int i = 2; i < 4; i++) {
		...
		if (i == 0 || i == 2) {
			r_um += 500.0;
			int x = intRound(m_cx + (r_um + 200) * COS[a[i]] * m_x_px_um);
			int y = intRound(m_cy - (r_um + 200) * SIN[a[i]] * m_y_px_um);
			CString s;
			s.Format(_T("%i°"), a[i]);
			int o = CheckAngle(a[i] - 91);
			int fs = intRound(0.085 * m_h);
			m_MemDC.WriteRotatedText(s, x, y, o, fs, 400, "Arial", Color, NOCOLOR);
		}
	}
}
...
//***************************************************************************************
//
void CEyeWnd::RedAstigmatismAxes()
{
	if (m_Type != TYPE_TWFM &&
		m_Type != TYPE_CWFM &&
		m_Type != TYPE_IWFM &&
	...
	if (m_Method3D) return;
	if (!m_MapShowMap) return;
	if (m_MapShowREDAstigmatismAxes != TRUE) return;
	if (m_astigm_axis == INVALID_VALUE) return;
		...
		if (m_MapShowMap || m_MapShowNumbers) {
			...
			if (v != INVALID_VALUE) {
				...
				if (m_Type == TYPE_TOPM)
					m_MemDC.WriteText(s, Rect, Font, white, 2);
				//531 Done
				else m_MemDC.WriteText(s, Rect, Font, m_Colors[i], 2);
			}
		}
	}
...
void CEyeWnd::OnMouseMove(uint nFlags, CPoint Point)
{
...
void CEyeWnd::OnLButtonDown(uint nFlags, CPoint Point)
{
...
void CEyeWnd::OnLButtonDblClk(uint nFlags, CPoint Point)
{
	if (m_MapShowMap && !m_Method3D) {
		...
		//   if(m_Type == TYPE_TOPM)
		//{
		//	s.Format(_T("Opacity Index = %.2f ± %.2fSD" , mean, std_dev);
		//}
		//else 
...
LRESULT CEyeWnd::OnChangePosMsg(WPARAM wParam, LPARAM lParam)
{
	if (!m_Method3D) {
...
void CEyeWnd::OnIncCentButtonClicked()
{
	m_Cent += m_Inc;
...
void CEyeWnd::OnDecCentButtonClicked()
{
	m_Cent -= m_Inc;
...
void CEyeWnd::OnZoomOutButtonClicked()
{
	m_Step += m_Inc;
...
void CEyeWnd::OnZoomInButtonClicked()
{
...
void CEyeWnd::OnOkulixButtonClicked()
{
	GetParent()->PostMessage(WM_OKULIX_LCLICK, 0, 0);
}
//521, Invoke Okulix function
